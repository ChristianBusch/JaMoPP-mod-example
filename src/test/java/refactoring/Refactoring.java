...
public class Refactoring {
	...
	public void test() {
		...
		// project org.emftext.language.java.resource has to be in workplace and buildpath
		...
		Resource resource = resSet.getResource(URI.createURI("src/test/java/input/CalculatorPow.java"), true); // load the file as resource
		CompilationUnit cu = (CompilationUnit) resource.getContents().get(0);
		List<Method> methods = cu.getContainedClass().getMethods();
		EObject content = methods.get(0).getFirstChildByType(LocalVariableStatement.class);
